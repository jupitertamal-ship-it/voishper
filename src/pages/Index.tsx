...
const STATS = [
  ...
  { icon: Star, label: 'Average Rating from Developers', value: '4.9/5' },
];
...
const DEMO_STEPS = [
  {
    ...
    desc: 'Drop a URL, upload a PDF, or paste raw text. Voishper ingests it all.',
    visual: 'ingest',
  },
  {
    title: 'Customize your AI',
    desc: 'Set the personality, brand colors, and greeting message in seconds.',
    visual: 'customize',
  },
  {
    title: 'Deploy with 1 line of code',
    desc: 'Copy one script tag. Paste into your site. Done.',
    visual: 'deploy',
  },
];
...
function DemoVisual({ step }: { step: string }) {
    ...
    return (
      <div className="w-full max-w-sm mx-auto">
        <motion.div
          ...
          className="glass-panel neon-border rounded-xl p-5 space-y-3"
        >
          ...
          <motion.div
            ...
            className="h-9 rounded-lg bg-muted/50 border border-border/50 flex items-center px-3 overflow-hidden"
          >
          ...
          <div className="border-t border-border/30 pt-3 flex items-center gap-3">
            <motion.div
              ...
              className="flex items-center gap-2 text-xs bg-muted/40 rounded-lg px-3 py-2 border border-border/30"
            >
    ...
    return (
      <div className="w-full max-w-sm mx-auto">
        <motion.div
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
          className="glass-panel neon-border rounded-xl p-5 space-y-4"
        >
          <div className="flex gap-3">
            {['#00F2FF', '#6366F1', '#F59E0B'].map((c, i) => (
              <motion.div
                key={c}
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
          ...
          >
            <div className="h-10 flex items-center px-3 gap-2" style={{ backgroundColor: '#6366F1' }}>
              <MessageSquare className="h-4 w-4 text-white" />
              <span className="text-white text-xs font-medium">Voishper Bot</span>
            </div>
            <div className="bg-card p-3">
              <div className="rounded-lg p-2.5 text-xs max-w-[75%]" style={{ backgroundColor: '#00F2FF20', color: 'hsl(var(--foreground))' }}>
  ...
  return (
    <div className="w-full max-w-sm mx-auto">
      ...
      >
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          <Code className="h-4 w-4 text-primary" /> Embed Code
        </div>
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ delay: 0.5 }}
          className="bg-muted/30 rounded-lg p-3 border border-border/30 font-mono text-xs leading-relaxed overflow-hidden"
        >
        ...
        >
          <div className="h-10 w-10 rounded-full flex items-center justify-center shadow-lg" style={{ backgroundColor: '#00F2FF', boxShadow: '0 0 20px #00F2FF40' }}>
    ...
    <div className="min-h-screen bg-background overflow-hidden relative">
      ...
      <nav className="relative z-10 flex items-center justify-between px-6 py-4 max-w-6xl mx-auto">
        <div className="flex items-center gap-2">
          ...
          <span className="text-lg font-bold text-gradient font-['Space_Grotesk']">Voishper AI</span>
        </div>
        <div className="flex gap-3">
      ...
      <section className="relative z-10 max-w-4xl mx-auto px-6 pt-16 md:pt-24 pb-12 text-center">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          ...
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-bold leading-tight mb-6 font-['Space_Grotesk']">
        ...
        >
          {STATS.map((stat, i) => (
            ...
            >
              ...
              <span className="text-2xl md:text-3xl font-bold font-['Space_Grotesk']">{stat.value}</span>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {FEATURES.map((f, i) => (
            <motion.div
              ...
              className="glass-panel neon-border p-6 hover:border-primary/40 transition-colors"
            >
              <div className="p-2 rounded-lg bg-primary/10 w-fit mb-4">
                <f.icon className="h-5 w-5 text-primary" />
              </div>
              <h3 className="font-semibold mb-2 font-['Space_Grotesk']">{f.title}</h3>
      ...
      <section className="relative z-10 max-w-5xl mx-auto px-6 pb-20">
        ...
        >
          <h2 className="text-3xl md:text-4xl font-bold font-['Space_Grotesk'] mb-4">
            Why <span className="text-gradient">Voishper</span>?
          </h2>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {WHY_ITEMS.map((item, i) => (
            <motion.div
              ...
              viewport={{ once: true, margin: '-50px' }}
              transition={{ delay: i * 0.15 }}
              className="glass-panel p-6 text-center hover:border-primary/30 border border-border/50 transition-colors rounded-xl"
            >
              <div className="p-3 rounded-xl bg-primary/10 w-fit mx-auto mb-4" style={{ boxShadow: '0 0 20px hsl(var(--primary) / 0.15)' }}>
                <item.icon className="h-6 w-6 text-primary" />
              </div>
              <h3 className="font-semibold mb-2 font-['Space_Grotesk']">{item.title}</h3>
      ...
      <section className="relative z-10 max-w-4xl mx-auto px-6 pb-24">
        <motion.div
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="glass-panel neon-border rounded-2xl p-10 md:p-16 text-center"
        >
          <h2 className="text-3xl md:text-4xl font-bold font-['Space_Grotesk'] mb-4">
            Ready to give your website a voice?
          </h2>
          <p className="text-muted-foreground mb-8 max-w-lg mx-auto">
            Join thousands of businesses already using Voishper AI to support customers 24/7.
          </p>
            ...
            >
                ...
                {DEMO_STEPS.map((_, i) => (
                  <div
                    ...
                    className={`h-1.5 rounded-full transition-all duration-300 ${
                      i === demoStep ? 'w-8 bg-primary' : 'w-3 bg-muted-foreground/30'
                    }`}
                  />
                ))}
              </div>
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  <div>
                    ...
                    <h3 className="text-2xl font-bold font-['Space_Grotesk'] mt-2">{DEMO_STEPS[demoStep].title}</h3>
              ...
              <div className="flex items-center justify-between mt-8">
                ...
                {demoStep < DEMO_STEPS.length - 1 ? (
                  <Button onClick={() => setDemoStep(demoStep + 1)} className="gap-1">
                    Next <ChevronRight className="h-4 w-4" />
                  </Button>
                ) : (
                  <Button onClick={() => { setShowDemo(false); navigate('/auth'); }} className="gap-1 neon-glow">
                    Get Started <ArrowRight className="h-4 w-4" />
                  </Button>
                )}
              </div>
